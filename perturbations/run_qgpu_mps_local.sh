...
METRICS_DIR="${METRICS_DIR:-$PERTURBATION_DIR/metrics}"
RAW_METRICS_DIR="$METRICS_DIR/raw"
SUMMARY_FILE="$METRICS_DIR/qgpu_mps_summary.tsv"
...
resolve_targets() {
    ...
    if [[ -n "$ONLY" ]]; then
        ...
        while IFS= read -r fep; do
            fep_names+=("$fep")
        done < <(find "$PERTURBATION_DIR/2.protein" -mindepth 2 -maxdepth 2 -type d -name inputfiles -printf '%h\n' | sed 's#.*/##' | sort)
    fi
    ...
    if ((${#fep_names[@]})); then
        ...
        for fep_name in "${fep_names[@]}"; do
            ...
            for system_dir in "${system_dirs[@]}"; do
                path="$PERTURBATION_DIR/$system_dir/$fep_name"
